...
```{r}
all_data <- dbGetQuery(con, "SELECT * from fires")
x <- all_data %>% select(STAT_CAUSE_DESCR, DISCOVERY_DOY, CONT_DOY, OWNER_DESCR, STATE, FIRE_SIZE_CLASS) %>% drop_na()
all_data[1,]
# y <- x$FIRE_SIZE_CLASS
...
#conf_mat(as.data.frame(preds), truth = FIRE_SIZE_CLASS, estimate = fire_size_class)
```
